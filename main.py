...
with open(csv_file_path, 'w', newline='') as csvfile:
    ...
    for indexx, numx in enumerate(results):
        ...
        csvwriter.writerow(["Index", "left top", "bottom right", "Horizontal diameter", "Vertical diameter", "Diameter_pixel","Circumference_pixels"])
        ...
        for indexy, numy in enumerate(results[indexx].boxes.xyxy):
            ...
            circumference_pixel=2*3.14*(diameter_pixel/2)

            # Increment the count
            cnt += 1

            # Writing the data to the CSV file
            csvwriter.writerow([cnt, left_top, bottom_right, horizontal_diameter, vertical_diameter, diameter_pixel, circumference_pixel])#,diameter_inches,circumference_feet,circumference_inches])
...
---------------------------------------------------------------------
'''
#"Diameter_pixel","Circumference_pixels"
# diameter_pixel, circumference_pixel
...
copy_diameter = original_image.copy()
...
for index, row in original_df.iterrows():
    ...
    circumference_inches = row["Circumference_pixels"]
    defect = row["Defect"]
    log_type = row["Diameter_pixel"]
    shape = row["Shape"]
    ...
    cv2.rectangle(copy_diameter, left_top, bottom_right, box_color, 2)
    ...
    text_size = cv2.getTextSize(str(circumference_pixel), font, font_scale, font_thickness)[0]
    text_origin = (left_top[0], int((left_top[1] + bottom_right[1] - text_size[1]) / 2))  # Middle left position

    # Draw text
    cv2.putText(copy_circumference, f"{circumference_pixel:.2f}", text_origin, font, font_scale, text_color, font_thickness, cv2.LINE_AA)
    cv2.putText(copy_defect, f"{defect}", text_origin, font, font_scale, text_color, font_thickness, cv2.LINE_AA)
    cv2.putText(copy_diameter, f"{diameter_pixel}", text_origin, font, font_scale, text_color, font_thickness, cv2.LINE_AA)
...
cv2.imwrite("/content/Circumference_pixels.jpg", copy_circumference)
cv2.imwrite("/content/Defect.jpg", copy_defect)
cv2.imwrite("/content/diameter_pixels.jpg", copy_diameter)
